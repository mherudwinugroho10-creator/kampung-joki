...
import { NegaraCell, TipeCell, TotalCell, ProfitCell, WorkerCell, DateCell, StatusBadge, ReqHeroCell } from '../components/TableCells'
import {
...
export default function Analytics() {
  ...
  const totalProfit = filtered.reduce((s,o)=>s+parseFloat(o.owner_price||0),0)
  const grossRev    = filtered.reduce((s,o)=>s+parseFloat(o.total_price||0),0)
  const keWorker    = filtered.reduce((s,o)=>s+parseFloat(o.worker_price||0),0)
  ...
  const monthlyData = useMemo(()=>{
    ...
    for(let i=5;i>=0;i--){
      ...
      result.push({ name:label, 'Total':mo.length, 'Selesai':mdone.length, 'Profit':mdone.reduce((s,o)=>s+parseFloat(o.owner_price||0),0) })
    }
    return result
  },[orders])
  ...
  const filterLabel = [
    filterMonth!=='all' ? MONTHS_ID[parseInt(filterMonth)-1] : null,
  ...
  return (
    <div style={{ width:'100%', minWidth:0 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', position:'relative', zIndex:1 }}>
          ...
          <div style={{ display:'flex', gap:10, alignItems:'center', flexWrap:'wrap', justifyContent:'flex-end' }}>
            ...
            <select value={filterMonth} onChange={e=>setFilterMonth(e.target.value)}
              ...
              {MONTHS_ID.map((m,i)=><option key={i} value={i+1} style={{ background:'#1e293b' }}>{m}</option>)}
            </select>
            {isFiltered && (
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:16, marginBottom:24 }}>
        {[
          { icon:BarChart3,  label:'Total Order',   value:totalOrder,      color:'#06B6D4', bg:'#CFFAFE' },
          { icon:TrendingUp, label:'Profit Owner',  value:fmt(totalProfit), color:'#8B5CF6', bg:'#EDE9FE' },
          { icon:Globe,      label:'Gross Revenue', value:fmt(grossRev),   color:'#10B981', bg:'#D1FAE5' },
          { icon:Award,      label:'Ke Worker',     value:fmt(keWorker),   color:'#F59E0B', bg:'#FEF3C7' },
        ].map((s,i)=>{
          const Icon=s.icon
          return (
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(280px, 1fr))', gap:20, marginBottom:20 }}>
        <div className="card fade-up" style={{ padding:24 }}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={monthlyData} barGap={3} barCategoryGap="38%" margin={{ top:4, right:8, left:8, bottom:0 }}>
        ...
        <div className="card fade-up">
          <div style={{ padding:'18px 22px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', gap:12 }}>
            ...
            <div>
              ...
              <p style={{ fontSize:12, color:'var(--muted)', marginTop:1 }}>{filtered.length} order · Gross {fmt(grossRev)} · Profit {fmt(totalProfit)}</p>
            </div>
          </div>
          <DataTable
            columns={[
